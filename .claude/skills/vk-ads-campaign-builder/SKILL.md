...
Пять правил. Механика инструментов (`vk_resolve_url`, контекстные фразы, замещение секций баннера, агентский `client_id`) — в `references/mcp-vk-workflow.md`, не здесь.

1. **Деньги — только по отдельному подтверждению.** Не включай показы, не ставь `status: active`, не возобновляй кампании/группы/объявления и не увеличивай бюджет без явного текстового «да» именно на это действие. Списание необратимо, и согласие на один шаг не переносится на следующий.
2. **Read → план → подтверждение → write → reread.** Перед любым write-tool прочитай текущее состояние через `vk_get_*`, покажи план, остановись до текстового подтверждения. После записи перечитай изменённые объекты и сообщи фактический результат. Без этого цикла легко создать дубль или сломать живую кампанию.
3. **Ноль выдуманных данных.** ID кампаний/групп/пакетов/целей/сегментов/регионов/счётчиков/форм, бюджеты, ставки, статистику и ограничения бери только из MCP или локальных docs. Нечего подтвердить — помечай как предположение. Ведём ID & facts ledger (фаза 3) — это предохранитель от галлюцинаций.
...
**Выход:** стратегия одним абзацем + список read-only проверок, которые проведёшь. Подтверди с человеком, прежде чем идти дальше.
...
**ID & facts ledger.** Выпиши каждый ID и каждую цифру, которые попадут в план, → рядом источник (какой `vk_get_*`). Пустая ячейка «Откуда» = значение не подтверждено = в план записи не идёт.
...
| segment_id / контекстные фразы | … | `vk_get_segments` / `vk_create_search_phrases` |
